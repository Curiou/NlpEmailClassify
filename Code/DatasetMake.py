...
from Code.CONFIG import DATASET_TWO_LABEL_DICT, TWO_TRAIN_ROOT_PATH, TWO_TEST_ROOT_PATH, TWO_TRAIN_H5_PATH, TWO_TEST_H5_PATH
# 数据集3的，导入标签对应关系，训练集目录，h5的保存路径
from Code.CONFIG import DATASET_THREE_LABEL_DICT, THREE_TRAIN_ROOT_PATH, THREE_TEST_ROOT_PATH, THREE_TRAIN_H5_PATH,THREE_TEST_H5_PATH
# 加载停止词
from Code.CONFIG import STOP_WORDS_FILE_PATH
        ...
        for subDir in subDirs:
            ...
            for file in os.listdir(os.path.join(rootDir, subDir)):
                ...
                with open(singleFilePath, encoding="utf8") as f:
                    content_str = f.read()
                DataFrame = DataFrame.append(pd.DataFrame({'content': [content_str], 'label': [label]}), ignore_index=True)
